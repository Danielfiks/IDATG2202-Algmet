 ...
 *  kapasitet/lengde (capacity) og det n�v�rende antallet (size).
 */
...
//          for (int i = 0; i < antall; i++)  data[i] = 0;  //  Un�dvendig?!!!
          antall = 0;
      }
      ...
      T get(const int pos) const {       //  Henter (om mulig) element nr.pos:
          if (pos >= 0  &&  pos < antall)  //  Innenfor bruksomr�de:
          ...
          return T();
                                ...
                                //  Smetter inn (om mulig) 't' p� plass 'pos':
      bool insert(const int pos, const T t) {
          if (antall == kapasitet)
              resize(kapasitet + 100);

          if (antall < kapasitet)  {        //  OPPGAVE: �ke arrayens lengde.
    ...
    // resize(kapasitet + 100);
      ...
      T pop_back() {                      //  Fjerner (om mulig) bakerste:
          if (!empty()) {                 //  Ikke tomt:
              T siste = data[antall-1];   //  Tar vare p� bakerste.
      ...
      T pop_front() {                     //  Fjerner (om mulig) forreste:
          if (!empty()) {                 //  Ikke tomt:
              T forste = data[0];         //  Tar vare p� forreste:
              remove(0);                  //  Fjerner den f�rste.
      ...
      bool remove(const int pos) {    //  Fjerner (om mulig) element nr.'pos':
          if (pos >= 0 && pos < antall) {             //  Lovlig indeks:
              for (int i = pos;  i < antall-1;  i++)  //  Flytter alle ETTERP�
      ...
      void resize(const int nyLengde) {   //  �ker (om mulig) arrayens lengde.
          ...
          if (nyLengde > kapasitet) {
              ...
              cout << "\n The new length has to be bigger than current capacity .....\n\n";
      }
      ...
      int  size() const  {  return antall;  }   // Antall n�v�rende elementer.
};
...
int main()  {
    ...
    cout << "capacity: " << sVec.capacity()
         << ",  size: " << sVec.size() << "\n";
    sVec.push_back("burger"); sVec.push_back("hamburger brod");
    sVec.push_front("ost"); sVec.push_front("hamburger brod");
    sVec.display();
